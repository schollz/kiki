...
Each user of the system is represented by an KiKi user name, which looks like
...
All the users are connected through a central key server at `key.kiki.io`,
...
`augie@kiki.io/Images/Augie/large.jpg`:
...
1. The client asks the key server for the record describing the owner of the
   file, which is the user name at the beginning of the file name (`augie@kiki.io`).
   The key server's response contains the name of the directory server holding
   that user's tree (`dir.kiki.io`) and Augie's public key.
2. The client asks the directory server for the directory entry describing the
   ...
   name of the store server (`store.kiki.io`).
