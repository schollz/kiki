...
func GetClientIPHelper(req *http.Request) (ipResult string, errResult error) {
	...
	if err == nil {
		log.Debugf("debug: Found IP using Request Headers sniffing. ip: %v", ip)
		return ip, nil
	}
	...
	if err == nil {
		log.Debugf("debug: Found IP using Request sniffing. ip: %v", ip)
		return ip, nil
	}
	...
	if err == nil {
		...
		if err == nil {
			log.Debugf("debug: Found IP using Header (Origin) sniffing. ip: %v", ip)
			return ip, nil
		}
	}
...
func getClientIPByRequestRemoteAddr(req *http.Request) (ip string, err error) {
	...
	if err != nil {
		log.Debugf("debug: Getting req.RemoteAddr %v", err)
		return "", err
	} else {
		log.Debugf("debug: With req.RemoteAddr found IP:%v; Port: %v", ip, port)
	}
	...
	log.Debugf("debug: Found IP: %v", userIP)
...
func getClientIPByHeaders(req *http.Request) (ip string, err error) {
	...
	for _, v := range ipSlice {
		log.Debugf("debug: client request header check gives ip: %v", v)
		if v != "" {
			return v, nil
		}
	}
