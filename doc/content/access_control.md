...
This document describes the access control model for KiKi.

All such information is maintained by files in the KiKi name space itself.
...
By user, we mean an account known to the KiKi Key service, identified by an
e-mail-like name: `ann@example.com`.
Each valid user has a *user* *root* directory held on at least one Directory
server.
Each user proves identity to KiKi servers,
...
Absent any other information, every item in the user's KiKi tree is readable
...
KiKi tree, only the owner may write them, and read access to the `Access`
files themselves is granted by the KiKi access control mechanisms described
here.
...
`Access` files may name any user or group in the KiKi system, including
...
Encrypted packings (described in the [KiKi Security document](/doc/security.md))  in
KiKi also have the effect of limiting who can read file contents, by only
...
Because of the semantics of I/O in KiKi, this is always wholesale replacement.
...
KiKi has no concept of "execute".
...
The user name `all` (case is ignored) means "any authenticated KiKi user".
The asterisked user name *@example.com means any authenticated user whose
account is in the `example.com` domain.

To allow anyone with an KiKi account to read items, this line in the relevant
Access file
...
the [KiKi Security document](/doc/security.md)), which is impractical at best.
In future, KiKi may provide a mechanism for some sort of key mechanism that
...
One goal of the design of access controls in KiKi is that a user cannot
easily discover valid names in the KiKi name space unless granted permission
to do so.
...
If the evaluation of an KiKi name reaches a link node, the Directory server
...
(These operations are handled automatically by the KiKi client library.)
...
modified with KiKi calls to the `DirServer`.
